...
namespace ApiGateway.Infrastructure.Controllers
{
    ...
    public class AnalyticsController : BaseController
    {
        ...
        )
        {
            request.LevelId = id;
            return await HandleAsync(
                async () => Ok(await _analyticsService.UpdateLevelAsync(request))
            );
        }
        ...
        public async Task<IActionResult> DeleteLevelAsync(string id)
        {
            ...
            return await HandleAsync(
                async () => Ok(await _analyticsService.DeleteLevelAsync(request))
            );
        }
        ...
        public async Task<IActionResult> GetLevelsAsync(
            [FromQuery] GetLevelsAnalyticsRequest request
        )
        {
            return await HandleAsync(
                async () => Ok(await _analyticsService.GetLevelsAsync(request))
            );
        }
    }
}
