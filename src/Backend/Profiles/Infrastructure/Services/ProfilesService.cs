...
namespace Profiles.Infrastructure.Services
{
    internal class ProfilesService : IProfilesServices
    {
        ...
        )
        {
            ...
            var result = await _subSkillRepository.UpdateAsync(
                Guid.Parse(request.SubSkillId!),
                subskill
            );
        ...
        )
        {
            ...
            return new DeleteAssessmentProfilesResponse
            {
                AssessmentId = result.AssessmentId.ToString()
            };
        }
            ...
            return new GetAssessmentsProfilesResponse
            {
                Assessments = results
                    .Select(assessment => new AssessmentProfiles
                    {
                        ...
                        Skills = assessment
                            .Role.RolePerSkills.Select(
                                rolePerSkill => new SkillWithSubSkillsProfiles
                                {
                                    ...
                                    SubSkills = rolePerSkill
                                        .Skill.SubSkills.Select(
                                            subskill => new SubSkillWithResultProfiles
                                            {
                                                SubSkillId = subskill.SubSkillId.ToString(),
                                                SkillId = subskill.SkillId.ToString(),
                                                Name = subskill.Name,
                                                Disabled = subskill.Disabled,
                                                Results = subskill.Results.Select(
                                                    result => new ResultProfiles
                                                    {
